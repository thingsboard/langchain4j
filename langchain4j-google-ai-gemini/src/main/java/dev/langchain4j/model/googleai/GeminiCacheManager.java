...
public class GeminiCacheManager {
    ...
    private static String getChecksum(GeminiContent content, List<GeminiTool> tools, GeminiToolConfig toolConfig) {
        ...
        if (tools != null) {
            List<GeminiTool> normalizedTools = tools.stream()
                    ...
                    .toList();
            sb.append(System.lineSeparator()).append(toCanonicalJson(normalizedTools));
        }
        if (toolConfig != null) {
            sb.append(System.lineSeparator()).append(toCanonicalJson(toolConfig));
        }
    ...
    private static String toCanonicalJson(Object o) {
        try {
            return HASH_MAPPER.writeValueAsString(o);
